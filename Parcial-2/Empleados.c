...
int Muestra1UnElementoId(eEmpleado * emp)
{
    int retorno=-1;
    if(emp !=NULL)
    {
        retorno=0;
        //printf("%s\t%s \n",emp->nombre,emp->direccion);//agregar geters
        printf("%d\t%s\t%s\n",get_id(emp),get_Nombre(emp),get_dire(emp));//,get_horas(emp), get_sueldo(emp));//agregar geters
//ver para sacarlo
...
void MostrarElementosId(ArrayList *this,char titulo[],int desde, int hasta)
{
    ...
    if(this!=NULL && desde >=0)
    {
        ...
        {//No esta vacio
            for(i=desde;i<hasta;i++)
            {
                Muestra1UnElementoId(al_get(this,i));
            }//fin for i
        }
        system("\npause");
    }
...
void MostrarElementosconSalario(ArrayList *this,int desde, int hasta)
{
   ...
   if(this !=NULL)
    {
        system("cls");
        printf("\n\n--------LISTADO DE EMPLEADOS CON SALARIO  --------\n");
...
int emp_buscarId(ArrayList* this, int cod)
{
    int retorno=0;
    int i;
    eEmpleado* emp;
